...
const Sections = () => {
  return (
    <div>
      ...
      <section id="about" className="relative py-16 sm:py-20 lg:py-24 bg-gradient-to-b from-gray-900 to-black overflow-hidden">
                ...
                <div className="space-y-4 sm:space-y-6">
                  <div className="flex items-center space-x-4">
                    ...
                    <div className="min-w-0">
                      ...
                      <p className="text-gray-400 text-sm sm:text-base break-all">{personalInfo.email}</p>
                    </div>
                  </div>
                  <div className="flex items-center space-x-4">
                    <div className="w-10 h-10 sm:w-12 sm:h-12 bg-gradient-to-r from-teal-500 to-cyan-500 rounded-full flex items-center justify-center flex-shrink-0">
                      <span className="text-white font-bold text-lg sm:text-xl">📱</span>
                    </div>
                    <div className="min-w-0">
                      <p className="font-semibold text-white text-sm sm:text-base">Phone</p>
                      <p className="text-gray-400 text-sm sm:text-base">{personalInfo.phone}</p>
                    </div>
                  </div>
                </div>
              </div>
            ...
            <AnimatedElement delay={400}>
              <div className="grid grid-cols-2 gap-4 sm:gap-6">
                {Object.entries(personalInfo.aboutStats).map(([key, value], idx) => (
                  <div key={idx} className="group relative p-4 sm:p-6 lg:p-8 rounded-xl sm:rounded-2xl bg-gray-800/50 border border-gray-700 hover:bg-gray-700/50 shadow-lg hover:shadow-2xl transition-all duration-500">
                    <div className="absolute inset-0 bg-gradient-to-br from-cyan-500/5 to-blue-500/5 rounded-xl sm:rounded-2xl opacity-0 group-hover:opacity-100 transition-opacity duration-500"></div>
      ...
      <section id="skills" className="relative py-16 sm:py-20 lg:py-24 bg-gradient-to-br from-black to-gray-900 overflow-hidden">
        <FloatingParticles />
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 sm:gap-8">
            {skills.map((skill, idx) => (
              <AnimatedElement key={idx} delay={idx * 100}>
                <div className="group relative p-6 sm:p-8 rounded-xl sm:rounded-2xl bg-gray-800/40 backdrop-blur-lg border border-gray-700 hover:bg-gray-700/40 transition-all duration-500">
                  <div className="absolute inset-0 bg-gradient-to-br from-cyan-500/10 to-blue-500/10 rounded-xl sm:rounded-2xl opacity-0 group-hover:opacity-100 transition-opacity duration-500"></div>
                  <div className="relative">
                    <div className="flex justify-between items-start mb-4">
                      <h3 className="text-lg sm:text-xl font-semibold text-white mb-2 leading-tight pr-2">{skill.name}</h3>
                      <span className="text-cyan-400 font-bold text-sm sm:text-base flex-shrink-0">{skill.level}%</span>
                    </div>
                    
                    <div className="w-full bg-gray-700 rounded-full h-2.5 sm:h-3 mb-4 overflow-hidden">
                      <div
      ...
      <section id="experience" className="relative py-16 sm:py-20 lg:py-24 bg-gradient-to-b from-gray-900 to-black">
                  ...
                  <div className="flex">
                    ...
                    <div className="flex-1 bg-gray-800/50 rounded-xl sm:rounded-2xl p-6 sm:p-8 shadow-lg hover:shadow-2xl transition-all duration-500 border border-gray-700">
                      ...
                      <div className="relative">
                        ...
                        <p className="text-gray-400 text-sm mb-4 sm:mb-6">{exp.period} | {exp.location}</p>
                        <p className="text-gray-300 mb-6 leading-relaxed text-sm sm:text-base">{exp.description}</p>
                        
                        <div className="flex flex-wrap gap-2">
                          {exp.technologies.map((tech, tid) => (
                            <span key={tid} className="px-3 py-1 bg-gradient-to-r from-cyan-500/20 to-blue-500/20 text-cyan-300 rounded-full text-xs sm:text-sm font-medium border border-cyan-500/30">
                              {tech}
                            </span>
                          ))}
                        </div>
      ...
      <section id="projects" className="relative py-16 sm:py-20 lg:py-24 bg-gradient-to-br from-black to-gray-900 overflow-hidden">
        <FloatingParticles />
        ...
        <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6 sm:gap-8">
            {projects.map((project, idx) => (
              <AnimatedElement key={idx} delay={idx * 200}>
                <div className="group relative bg-gray-800/40 backdrop-blur-lg rounded-xl sm:rounded-2xl p-6 sm:p-8 border border-gray-700 hover:bg-gray-700/40 transition-all duration-500 hover:scale-105">
                  <div className="absolute inset-0 bg-gradient-to-br from-cyan-500/10 to-blue-500/10 rounded-xl sm:rounded-2xl opacity-0 group-hover:opacity-100 transition-opacity duration-500"></div>
                  <div className="relative">
                    <div className="w-10 h-10 sm:w-12 sm:h-12 bg-gradient-to-r from-cyan-500 to-blue-500 rounded-lg sm:rounded-xl flex items-center justify-center mb-6 group-hover:rotate-12 transition-transform duration-300">
                    ...
                    <div className="flex flex-wrap gap-2 mb-6">
                      {project.tech.slice(0, 4).map((tech, tid) => (
                        <span key={tid} className="px-2 py-1 sm:px-3 bg-gray-700 text-gray-200 rounded-full text-xs sm:text-sm font-medium">
                          {tech}
                        </span>
                      ))}
                      {project.tech.length > 4 && (
                        <span className="px-2 py-1 sm:px-3 bg-gray-700 text-gray-200 rounded-full text-xs sm:text-sm font-medium">
